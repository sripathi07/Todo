            ...
            <div className="col-md-6">
                ...
                <ul className="list-group">
                    ...
                    <div className="d-flex gap-2">
                        { editId == -1 ?<button className="btn btn-danger" onClick={() => handleEdit(item)} >Edit</button>:<button className="btn btn-warning" onClick={handleUpdate} >Update</button>}
    ...
    const handleSubmit = () => {
        ...
        if (title.trim() !== '' && description.trim() !== '') {
            fetch(apiUrl + "/todos", {
                ...
                if (res.ok) {
                    // Add item to list
                    setTodos([...todos, { _id: Date.now(), title, description, dateTime }]); // Assuming the backend returns a unique ID
                    setTitle("");
                    setDesciption("");
                    setMessage("Item added successfully");
                    setTimeout(() => {
    ...
    const handleUpdate = () => {
        ...
        if (editTitle.trim() !== '' && editDescription.trim() !== '') {
            fetch(apiUrl + "/todos/" + editId, {
                ...
                body: JSON.stringify({ title: editTitle, description: editDescription, dateTime: editDateTime })
            }).then((res) => {
                if (res.ok) {
                    // Update item to list
                    const updatedTodos = todos.map((item) => {
                                ...
                                <div className="d-flex flex-column me-2">
                                    {editId === -1 || editId !== item._id ? (
                                        ...
                                        <div className="form-group d-flex gap-2">
                                            <input placeholder="Title" onChange={(e) => setEditTitle(e.target.value)} value={editTitle} className="form-control" type="text" />
                                            <input placeholder="Description" onChange={(e) => setEditDesciption(e.target.value)} value={editDescription} className="form-control" type="text" />
                                            <input placeholder="Date and Time" onChange={(e) => setEditDateTime(e.target.value)} value={editDateTime} className="form-control" type="text" />
                                        </div>
                                    )}
                                </div>
                                <div className="d-flex gap-2">
